...
export const createActivity = async (req, res) => {
  try {
    ...
    // Handle AI Planner’s simplified payload (e.g. { name: "Hiking (2 hours)" })
    if (!activityName && req.body.name) {
        activityName = req.body.name;
        durationMinutes = 0;
        targetMinutes = req.body.targetMinutes || 60;
        if (typeof targetMinutes === 'string') {
  const match = targetMinutes.match(/(\\d+)/);
  if (match) targetMinutes = parseInt(match[1]) * (targetMinutes.includes('hour') ? 60 : 1);
}

        taskType = req.body.taskType || 'Weekly'; // frontend can now decide
    ...
    if (!activityName || durationMinutes === undefined || !date || !taskType || !targetMinutes) {
      return res.status(400).json({
        message:
          'Please provide activityName, durationMinutes, date, taskType, and targetMinutes.',
      });
    }
